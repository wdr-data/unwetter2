...
def parse_xml(xml):
    ...
    for field_xml, field_json in fields.items():
        ...
        if field_json in datetime_fields and event[field_json]:
            event[field_json] = (
                iso8601.parse_date(event[field_json])
                .astimezone(pytz.utc)
                .replace(tzinfo=None)
            )
    ...
    event["parameters"] = {
        param["valueName"]: param["value"] for param in xml_dict["info"]["parameter"]
    }
    ...
    for area in xml_dict["info"]["area"]:
        ...
        area["_exclude_polygons"] = [
            parse_polygon(poly) for poly in area["_exclude_polygons"]
        ]
    ...
    if "references" in xml_dict:
        event["references"] = [
            ref.split(",")[1] for ref in xml_dict["references"].split(" ")
        ]

    if event["msg_type"] == "Update":
        old_events = list(
            db.by_ids(event.get("extended_references", event["references"]))
        )

        if not any(e["published"] for e in old_events):
            extended_references = set()
            extended_references.update(
                event.get("extended_references", event["references"])
            )
